...
class MinecraftReinforce(Reinforce):
    ...
    def calculateGradient(self):
        ...
        loglhs = [sum(self.loglh['loglh'][int(seqidx[n]):int(seqidx[n + 1]), :]) for n in range(self.dataset.getNumSequences() - 1)]
        # append sum of last sequence as well
        loglhs.append(sum(self.loglh['loglh'][int(seqidx[-1]):, :]))
        loglhs = array(loglhs)
